...
Konfigurasi FreeBSD dan DNS Unbound Untuk Melakukan Caching DNSSEC dan
DOT
:::

::: {style="text-align: justify;"}
CONFIGURATION UNBOUND FOR CACHING DNSSEC AND DOT WITH FREEBSD.txt
:::
...
::: {style="text-align: justify;"}
The DNS service or Domain Name System is a basic service of the
Internet, as well as other networks that operate based on the TCP/IP
protocol family, and is used to obtain matching host names on the
network with corresponding digital addresses. Despite such a simple
description, DNS is perhaps the most complex network service in terms of
its structure and set of interactions, and its reliable operation
depends on the reliable operation of everyone and everything.
:::
...
::: {style="text-align: justify;"}
DNSSEC provides authentication of DNS records using digital signatures,
which protects them from possible replacement. However, at the same
time, all data is always transmitted in clear form and is not protected
from view during transit in any way. In the absence of a digital
signature, if desired, it can be easily modified for various purposes -
from criminal to censorship.
:::
...
::: {style="text-align: justify;"}
This article explains how to configure and use the Unbound DNS server,
both as caching, DNSSEC and as DNS over TLS.
:::
...
::: {style="text-align: justify;"}
Unbound is a highly secure validating, recursive, and caching DNS server
developed by NLnet Labs, VeriSign Inc, Nominet, and Kirei. This software
is distributed free of charge under the BSD license. Binaries are
written with a high security focus and very strict C code.
:::
...
::: {style="text-align: justify;"}
To help improve online privacy, Unbound supports DNS-over-TLS and
DNS-over-HTTPS allowing clients to encrypt their communications. In
addition, it supports various modern standards that limit the amount of
data exchanged with official servers. This standard not only improves
privacy but also helps make DNS more robust. The most important are
Query Name Minimisation, Aggressive Use of DNSSEC-Validated Cache and
support for authority zones, which can be used to load a copy of the
root zone.
:::
...
**[1.Â ]{style="font-size: medium;"}**[**System
Specifications**]{style="font-size: medium;"}
...
**[2. Instalasi Unbound]{style="font-size: medium;"}**
...
::: {style="text-align: justify;"}
Generally, to install Unbound on a FreeBSD server, you can do two
things, namely ports and pkg. In this article, we will discuss
...
::: {style="text-align: justify;"}
The first step, please log in to your FreeBSD server via the FreeBSD
console or remote SSH with PuTTy, then run the following command lines:
:::
...
``` {style="background: rgb(253, 246, 227); border-color: rgba(191, 19, 19, 0.51); border-image: initial; border-style: solid; border-width: 1px 1px 1px 8px; box-sizing: border-box; direction: ltr; font-feature-settings: normal; font-kerning: auto; font-optical-sizing: auto; font-stretch: normal; font-variant-alternates: normal; font-variant-east-asian: normal; font-variant-numeric: normal; font-variation-settings: normal; line-height: 19px; list-style: none; margin-bottom: 20px; margin-top: 0px; max-height: 250px; outline: none; overflow: visible scroll; padding: 1em; text-align: start;"}
...
After finishing updating the pkg package, we continue with installing
unbound, type the following command to install unbound.
...
::: {style="text-align: justify;"}
So that the unbound server can run automatically every time the FreeBSD
server is turned off or restarted, you must first add the following
command to rc.conf, by typing the script below in the /etc/rc.conf file.
:::
...
After that, in the \"resolv.conf\" file, enter the script below. Use the
default FreeBSD application, namely \"ee\" to add the script.
:::
...
::: {.separator style="clear: both;"}
Edit the hosts file, use the command ee /etc/hosts, then enter the
following syntax.
:::
...
127.0.0.1Â Â  Â Â Â localhost localhost.miner4pool.org
...
::: {.separator style="clear: both;"}
**[3.Â ]{style="font-size: medium;"}**[**Unbound
Configuration**]{style="font-size: medium;"}
:::
...
Before we start configuring unbound.conf, the unbound application
requires a \"root.hints\" file that lists the primary DNS Server. The
Unbound DNS Server application comes with a list of root DNS Servers in
its code, but it ensures an up-to-date copy on each server. A good
practice is to update this file every six months.
...
By default the unbound.conf file is in the /usr/local/etc/unbound
directory. In the Putty console, type the following command.
...
[Additionally, Unbound requires an auto-trust-anchor file. This file
contains the keys so that DNSSEC can be validated. To generate the
root.key, run the following command in the putty
console.]{style="text-align: justify;"}
:::
...
::: {style="text-align: left;"}
[The next step is to create the necessary keys for Unbound to be
controlled by unbound-control, type the command below to run unbound
control setup.]{style="text-align: justify;"}
:::
...
The next step is to edit the unbound.conf file, which is located at
...
Before we edit the unbound.conf file, in this section we will divide it
...
1\. Unbound Server as DNS Caching

</div>

<div>

2\. Server Unbound as DNS Caching and DNS Over TLS
...
This was deliberately created to make it easier for readers to
understand and study the Unbound server.
...
**[a.Â ]{style="font-size: medium;"}**[**Unbound Server as DNS
Caching**]{style="font-size: medium;"}

</div>

<div>

[So that unbound can function as DNS caching, we have to edit the
unbound.conf file. Type the following command in the putty
console.]{style="font-size: medium;"}
...
Below is a complete example script from the \"unbound.conf\" file
...
``` {style="background-color: white; border: 1px solid rgb(209, 209, 209); box-sizing: inherit; line-height: 1.3125; margin-bottom: 1.75em; margin-top: 0px; max-width: 100%; overflow-wrap: break-word; overflow: auto; padding: 1.75em; text-align: start;"}
#
...
::: {style="text-align: start;"}
After the unbound.conf file editing process is complete, restart the
unbound server with the command:
:::
...
;; ANSWER SECTION:
google.com.     269 IN  A   74.125.200.101
google.com.     269 IN  A   74.125.200.139
google.com.     269 IN  A   74.125.200.102
google.com.     269 IN  A   74.125.200.113
google.com.     269 IN  A   74.125.200.100
google.com.     269 IN  A   74.125.200.138

;; Query time: 368 msec
;; SERVER: 192.168.9.3#53(192.168.9.3) (UDP)
;; WHEN: Mon Oct 16 16:53:41 WIB 2023
;; MSG SIZE  rcvd: 135
```
:::
...
::: {.separator style="clear: both;"}
**[b.Â ]{style="font-size: medium;"}**[**Unbound Server as DNS Caching &
DNS Over TLS**]{style="font-size: medium;"}
:::
...
Another more modern way to protect DNS traffic is the DNS-over-TLS
protocol described in the RFC7858 standard, which is data encapsulation
in standard TLS. We recommend using port 853 for access. Just like
DNSCrypt, it is assumed that the DNS client, which is usually the same
local caching DNS, accesses a remote server that supports DNS-over-TLS.
...
The aforementioned Unbound has built-in support for this protocol, so no
additional software layer is required to use, as is the case with
DNSCrypt.
...
In order for Unbound to run as DNS-over-TLS, Unbound only requires a
list of servers in the forward zone, the same as the configuration file
above, but there are a few things that need to be changed. Setting up
Unbound as a server serving DNS-over-TLS requests is also easy. To do
this, you can use an existing TLS certificate, for example openssl.
...
The first step that must be taken to implement unbound as a DNS Over TLS
server is to create an SSL certificate. We will place this unbound SSL
file in the /etc/ssl/unbound directory. To do this, in the Putty console
...
::: {style="font-size: large;"}
After we have successfully created the SSL file, the next step is to
change the unbound.conf file above. Type the following command in the
putty console.
:::
...
;; ANSWER SECTION:
google.com.     109 IN  A   172.253.118.139
google.com.     109 IN  A   172.253.118.138
google.com.     109 IN  A   172.253.118.100
google.com.     109 IN  A   172.253.118.101
google.com.     109 IN  A   172.253.118.113
google.com.     109 IN  A   172.253.118.102

;; Query time: 173 msec
;; SERVER: 192.168.9.3#853(192.168.9.3) (UDP)
;; WHEN: Fri Oct 20 10:35:51 WIB 2023
;; MSG SIZE  rcvd: 135
```
:::

::: {.separator style="clear: both;"}
The answer is the server: 192.168.9.3#853, this means that unbound NDS
Over TLS port 853 is active. Type the following command to view open DNS
ports:
:::
...
From the open ports above, we can see that port 853 and port 53 are
open. This indicates Server Unbound DNS Over TLS RUNNING.
...
**[4.Â ]{style="font-size: medium;"}**[**Create an Unbound Log
File**]{style="font-size: medium;"}
...
The final step, we will create a log file. In the Putty console, type
the command below and enter the script in the /etc/newsyslog.conf file.
...
The next step, we create a log_reopen file and enter the following
script in the log_reopen.
...
``` {style="background: rgb(253, 246, 227); border-color: rgba(191, 19, 19, 0.51); border-image: initial; border-style: solid; border-width: 1px 1px 1px 8px; box-sizing: border-box; direction: ltr; font-feature-settings: normal; font-kerning: auto; font-optical-sizing: auto; font-stretch: normal; font-variant-alternates: normal; font-variant-east-asian: normal; font-variant-numeric: normal; font-variation-settings: normal; line-height: 19px; list-style: none; margin-bottom: 20px; margin-top: 0px; max-height: 250px; outline: none; overflow: visible scroll; padding: 1em; text-align: start;"}
root@miner4:~ # ee /usr/local/etc/unbound/log_reopen
...
DNS over TLS or DNS over TCP, but wrapped within a TLS session, will
encrypt your requests and the server\'s replies, and optionally allow
you to verify the server\'s identity.
...
The advantage is protection against eavesdropping and manipulation of
your DNS traffic. the downside is a slight performance degradation and
potential firewall traversal issues, as it runs over a non-standard port
(TCP port 853) that may be blocked on some networks.
...
This tutorial has helped you in setting up and configuring Unbound DNS
Over TLS. These tutorials are discussed in detail and structured, with
the hope that you can easily understand them.
