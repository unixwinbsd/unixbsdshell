Konfigurasi FreeBSD dan DNS Unbound Untuk Melakukan Caching DNSSEC dan DOT
...
Before we edit the unbound.conf file, in this section we will divide it into 3 sessions, namely:
...
server:
	...
	# tls-ciphersuites: "TLS\_AES\_128\_GCM\_SHA256:TLS\_AES\_128\_CCM\_8\_SHA256:TLS\_AES\_128\_CCM\_SHA256:TLS\_AES\_256\_GCM\_SHA384:TLS\_CHACHA20\_POLY1305\_SHA256"
...
In order for Unbound to run as DNS-over-TLS, Unbound only requires a list of servers in the forward zone, the same as the configuration file above, but there are a few things that need to be changed. Setting up Unbound as a server serving DNS-over-TLS requests is also easy. To do this, you can use an existing TLS certificate, for example openssl.

  

The first step that must be taken to implement unbound as a DNS Over TLS server is to create an SSL certificate. We will place this unbound SSL file in the /etc/ssl/unbound directory. To do this, in the Putty console menu, type the following command.
...
The answer is the server: 192.168.9.3#853, this means that unbound NDS Over TLS port 853 is active. Type the following command to view open DNS ports:
...
The final step, we will create a log file. In the Putty console, type the command below and enter the script in the /etc/newsyslog.conf file.
...
DNS over TLS or DNS over TCP, but wrapped within a TLS session, will encrypt your requests and the server's replies, and optionally allow you to verify the server's identity.

  

The advantage is protection against eavesdropping and manipulation of your DNS traffic. the downside is a slight performance degradation and potential firewall traversal issues, as it runs over a non-standard port (TCP port 853) that may be blocked on some networks.

  

This tutorial has helped you in setting up and configuring Unbound DNS Over TLS. These tutorials are discussed in detail and structured, with the hope that you can easily understand them.
