...
To start installing phpLDAPadmin, you can follow the steps below.

```
root@ns1:~ # cd /usr/ports/net/php82-ldap
root@ns1:/usr/ports/net/php82-ldap # make install clean
root@ns1:/usr/ports/net/php82-ldap # cd /usr/ports/net/phpldapadmin
root@ns1:/usr/ports/net/phpldapadmin # make install clean
```

Now you open the /usr/local/etc/apache24/httpd.conf file again and enter the script below at the very bottom of the script file /usr/local/etc/apache24/httpd.conf.
...
After that, open and edit the file /usr/local/www/phpldapadmin/config/config.php and enter the following script.

```
$servers->setValue('server','name','unixexplore.com');
$servers->setValue('server','host','192.168.5.2');
$servers->setValue('server','port',389);
$servers->setValue('server','base',array('dc=unixexplore,dc=com'));
$servers->setValue('login','bind_id','cn=Manager,dc=unixexplore,dc=com');
$servers->setValue('login','bind_pass','semeru');
```

The script above adapts to the settings of the OpenLDAP application program that you have installed and configured. For more details, below is the complete script from the file /usr/local/www/phpldapadmin/config/config.php.
